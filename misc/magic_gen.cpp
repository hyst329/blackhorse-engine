...
uint64 rookAttacksTable[64][4096];
uint64 bishopAttacksTable[64][512];
...
uint64 find_magic(int sq, int m, int bishop)
{
    ...
    n = count_1s(mask);
    printf("Mask n for square %d is %d (equal %d)\n", sq, n, m == n);
    ...
    for (k = 0; k < 100000000; k++)
    {
        ...
        if (!fail)
        {
            memcpy(bishop ? bishopAttacksTable[sq] : rookAttacksTable[sq], used, 1 << (m + 3)); // 8 times greater than the size
            return magic;
        }
    }
    printf("***Failed***\n");
...
int main()
{
    ...
    fprintf(mg, "#ifndef MAGIC_H\n");
    fprintf(mg, "#define MAGIC_H\n");
    fprintf(mg, "struct MaskedMagic\n{\n    uint64_t mask, magic;\n};\n\n");

    fprintf(mg, "const MaskedMagic ROOK_MAGIC[64] = {\n");
    for (square = 0; square < 64; square++)
        fprintf(mg, "    {0x%016llXULL, 0x%016llXULL},\n", rmask(square), find_magic(square, RBits[square], 0));
    fprintf(mg, "};\n\n");

    fprintf(mg, "const MaskedMagic BISHOP_MAGIC[64] = {\n");
    for (square = 0; square < 64; square++)
        fprintf(mg, "    {0x%016llXULL, 0x%016llXULL},\n", bmask(square), find_magic(square, BBits[square], 1));
    fprintf(mg, "};\n\n");

    fprintf(mg, "#endif // MAGIC_H\n");
    for (square = 0; square < 64; square++)
    {
        fwrite(rookAttacksTable[square], sizeof(uint64), 4096, ra);
        fwrite(bishopAttacksTable[square], sizeof(uint64), 512, ba);
    }
    fclose(mg);
    fclose(ra);
    fclose(ba);
