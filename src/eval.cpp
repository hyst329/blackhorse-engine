...
const int16_t PAWN_EVAL_TABLE[SQUARES_COUNT] = {
    0, 5,   5,   0,  5,  10, 50, 0, 0, 10,  -5,  0,  5,  10, 50, 0,
    0, 10,  -10, 0,  10, 20, 50, 0, 0, -20, 0,   20, 25, 30, 50, 0,
    0, -20, 0,   20, 25, 30, 50, 0, 0, 10,  -10, 0,  10, 20, 50, 0,
    0, 10,  -5,  0,  5,  10, 50, 0, 0, 5,   5,   0,  5,  10, 50, 0,
};

const int16_t KNIGHT_EVAL_TABLE[SQUARES_COUNT] = {
    -50, -40, -30, -30, -30, -30, -40, -50, -40, -20, 5,   0,   5,
    0,   -20, -40, -30, 0,   10,  15,  15,  10,  0,   -30, -30, 5,
    15,  20,  20,  15,  0,   -30, -30, 5,   15,  20,  20,  15,  0,
    -30, -30, 0,   10,  15,  15,  10,  0,   -30, -40, -20, 5,   0,
    5,   0,   -20, -40, -50, -40, -30, -30, -30, -30, -40, -50,
};

const int16_t BISHOP_EVAL_TABLE[SQUARES_COUNT] = {
    -20, -10, -10, -10, -10, -10, -10, -20, -10, 5,   10,  0,   5,
    0,   0,   -10, -10, 0,   10,  10,  5,   5,   0,   -10, -10, 0,
    10,  10,  10,  10,  0,   -10, -10, 0,   10,  10,  10,  10,  0,
    -10, -10, 0,   10,  10,  5,   5,   0,   -10, -10, 5,   10,  0,
    5,   0,   0,   -10, -20, -10, -10, -10, -10, -10, -10, -20,
};

const int16_t ROOK_EVAL_TABLE[SQUARES_COUNT] = {
    0, -5, -5, -5, -5, -5, 5,  0, 0, 0,  0,  0,  0,  0,  10, 0,
    0, 0,  0,  0,  0,  0,  10, 0, 5, 0,  0,  0,  0,  0,  10, 0,
    5, 0,  0,  0,  0,  0,  10, 0, 0, 0,  0,  0,  0,  0,  10, 0,
    0, 0,  0,  0,  0,  0,  10, 0, 0, -5, -5, -5, -5, -5, 5,  0,
};

const int16_t QUEEN_EVAL_TABLE[SQUARES_COUNT] = {
    -20, -10, -10, 0, -5, -10, -10, -20, -10, 0,   5,   0,  0,  0,   0,   -10,
...
const int16_t KING1_EVAL_TABLE[SQUARES_COUNT] = {
    20, 20, -10, -20, -30, -30, -30, -30, 30, 20, -20, -30, -40, -40, -40, -40,
    10, 0,  -20, -30, -40, -40, -40, -40, 0,  0,  -20, -40, -50, -50, -50, -50,
    0,  0,  -20, -40, -50, -50, -50, -50, 10, 0,  -20, -30, -40, -40, -40, -40,
    30, 20, -20, -30, -40, -40, -40, -40, 20, 20, -10, -20, -30, -30, -30, -30,
};

const int16_t KING2_EVAL_TABLE[SQUARES_COUNT] = {
    -50, -30, -30, -30, -30, -30, -30, -50, -30, -30, -10, -10, -10,
    -10, -20, -40, -30, 0,   20,  30,  30,  20,  -10, -30, -30, 0,
    30,  40,  40,  30,  0,   -20, -30, 0,   30,  40,  40,  30,  0,
    -20, -30, 0,   20,  30,  30,  20,  -10, -30, -30, -30, -10, -10,
    -10, -10, -20, -40, -50, -30, -30, -30, -30, -30, -30, -50,
};

// TODO : Make use of KING2
const int16_t *EVAL_TABLES[PIECES_COUNT + 1] = {
    NONE_EVAL_TABLE, PAWN_EVAL_TABLE,  KNIGHT_EVAL_TABLE, BISHOP_EVAL_TABLE,
    ROOK_EVAL_TABLE, QUEEN_EVAL_TABLE, KING1_EVAL_TABLE,
};
...
bool helper_compare(Board &board, Move &bestmove, Move &m1, Move &m2) {
  ...
  return (10 * captured1_piece - capturing1_piece) >
         (10 * captured2_piece - capturing2_piece);
}
...
int16_t EvaluationEngine::evaluate_final(Board &board) {
  ...
  bool endgame =
      (pieces_count <= 18) || ((board.get_bitboard(WHITE * QUEEN) |
                                board.get_bitboard(BLACK * QUEEN)) == 0 &&
                               pieces_count <= 24);

  for (int i = 0; i < SQUARES_COUNT; i++) {
    int8_t piece = board.get_piece((Square)i);
    const int16_t *eval_table = (endgame && abs(piece) == KING)
                                    ? KING2_EVAL_TABLE
...
int16_t EvaluationEngine::evaluate_depth(
    Board &board, int depth, int odepth, int16_t alpha, int16_t beta,
    bool show_moves, ostream &output,
    map<uint64_t, pair<int16_t, int>> &hash_table,
    map<uint64_t, Move> &hash_var, clock_t scheduled, Move bestmove) {
  ...
  if (moves.empty()) {
    if (MoveGenerator::detect_check(board)) {
      int16_t checkmate_score =
          board.get_side_to_move() == WHITE ? MIN_SCORE : MAX_SCORE;
  ...
  sort(moves.begin(), moves.end(), [&board, &bestmove](Move m1, Move m2) {
    return helper_compare(board, bestmove, m1, m2);
  });
  int index = 0;
  for (Move m : moves) {
    if (show_moves) {
      output << "info currmove " << m << " currmovenumber " << (++index)
             << endl;
    }
    // cout << depth << m << endl;
    ...
    hash_table[board.get_hash()].second > odepth - depth)
    {
    ...
    else*/
    {
      ...
      er = -evaluate_depth(board, depth - 1, odepth, -beta, -alpha, false,
                           output, hash_table, hash_var, scheduled, m0);
...
int16_t EvaluationEngine::evaluate_quiesce(Board &board, int16_t alpha,
                                           int16_t beta) {
  vector<Move> legal_captures =
      MoveGenerator::generate_moves_legal(board, true);
  // if (legal_moves.empty())
  // {
  // 	if (MoveGenerator::detect_check(board))
  // 	{
  // 		int16_t checkmate_score = board.get_side_to_move() == WHITE ?
  // MIN_SCORE : MAX_SCORE; 		return checkmate_score;
  // 	}
  // 	else
  // 	{
  // 		return 0;
