    ...
    if (uci_initialised) {
      ...
      if (command == "position") {
        ...
        position_description =
            position_description.substr(pdstart, pdend - pdstart + 1);
        ...
        if (!moves.empty()) {
          ...
          while (iss >> move_str) {
            ...
            vector<Move> legal_moves =
                MoveGenerator::generate_moves_legal(board);
            auto it =
                find_if(legal_moves.begin(), legal_moves.end(), [m](Move n) {
                  return n.get_from() == m.get_from() &&
                         n.get_to() == m.get_to();
                });
      ...
      if (command == "go") {
        ...
        thread t([this, depth, max_time]() {
          return output_thinking(depth, max_time);
        });
        t.detach();
      }
      if (command == "stop") {
...
void UCI::output_thinking(int max_depth, int max_time) {
  ...
  for (int i = 1; i <= max_depth; i++) {
    ...
    int16_t er = EvaluationEngine::evaluate(board, i, true, output, hash_table,
                                            hash_var, scheduled, bestmove);
