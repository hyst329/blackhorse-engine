...
class Move {
public:
  Move(Square from, Square to, int8_t captured_piece = 0,
       int8_t promoted_piece = 0, bool en_passant = false)
      : from(from), to(to), captured_piece(captured_piece),
        promoted_piece(promoted_piece), en_passant(en_passant) {}
  ...
  bool operator==(Move &other) { return from == other.from && to == other.to; }
...
private:
  template <int piece>
  static void generate_moves_single_piece(const Board &board, Square square,
                                          vector<Move> &target,
                                          bool captures_only = false);
};
